...
<body style="background-color: white;">
      <div id="google_translate_element" align="right"></div>
         ...
         if(isset($_POST['submit'])){
            $state = $_POST['state'];
 $data = mysqli_query($con,"Select schstate From schemes Where schstate = '$state' ");
            while ($row1 = mysqli_fetch_array($data)) {
               ?>
      <h1 style="margin-top: 35px; color: blue;"><center><b>
     <?php echo $row1['schstate']; ?> GOVERNMENT HEALTH SCHEMES</b></center> </h1>
     ...
     <input type="submit" name="submit" value="submit" onclick="myFunction()">
          ...
          <?php
           ...
           if(isset($_POST['submit'])){
            $state = $_POST['state'];
           $data=mysqli_query($con,"SELECT `id`, `schname`, `schobj`, `schelg`, `schdetails`, `schform`, `schhosp`, `scfund`, `schstate` FROM `schemes` WHERE schstate = '$state' ");
